        ...
        <div className='lg:col-span-7 space-y-8'>
          ...
          <div className='space-y-3'>
            ...
            <h1 className='text-4xl sm:text-5xl text-slate-800 leading-tight font-medium'>
              Highly Secure, End-to-End Encrypted Video Conferencing for Everyone. <br />
              <span className='text-primary'>Built for Everyone.</span>
            </h1>

            <p className='text-slate-700 text-base sm:text-lg max-w-xl leading-relaxed'>Connect, Collabrate, and Celebrate from anywhere with ultra-low latency video, screen shairing, and real-time chat.
            </p>

            <div className='flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-2'>

              <button onClick={handleCreateMeeting} disabled={isCreating} className='bg-primary hover:bg-primary-hover text-white font-medium px-6 py-3.5 rounded-full shadow-md shadow-primary/20 flex items-center gap-2.5 justify-center transition-all cursor-pointer disabled:opacity-50'>
              ...
              <form onSubmit={handleJoinMeeting} className='flex-1 flex items-center gap-2'>
                <div className='flex-1 relative'>
                  ...
                  <input type="text" placeholder="Enter Meeting ID (e.g. abc-def-ghi)" value={joinId} onChange={(e) => setJoinId(e.target.value)} className="w-full bg-white/75 border border-primary-border/80 focus:border-primary/60 focus:ring-1 focus:ring-primary/60 rounded-full pl-12 pr-4 py-3.5 text-sm text-slate-800 placeholder-slate-400 outline-none transition-all"/>

                </div>

                <button type="submit"
                disabled={!joinId.trim()}
                className='bg-primary hover:bg-primary disabled:opacity-40 disabled:hover:bg-slate-900 text-white font-medium px-6 py-3.5 rounded-full transition-all flex items-center justify-center cursor-pointer shadow-xs'>
                  <span>Join</span>

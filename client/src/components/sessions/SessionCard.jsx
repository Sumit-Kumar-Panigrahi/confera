    ...
    <div className="bg-white/70 backdrop-blur rounded-3xl p-6 transition-all flex flex-col justify-between space-y-5 border border-slate-100/60 shadow-xs">
      ...
      <div className="flex items-center justify-between gap-3 pt-2">
        <button onClick={()=> onOpenDetails(session.id)}
          className='w-full bg-slaate-400/10 hover:bg-slate-400/20 text-slate-800 font-medium py-2.5 px-4 rounded-full text-xs transition-all cursor-pointer text-center'>
          View Details
        </button>
        {!isEnded && (

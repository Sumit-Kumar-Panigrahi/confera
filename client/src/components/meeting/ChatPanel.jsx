     ...
     return(
      <aside className='w-full sm:w-80 h-full bg-white border-1 border-slate-200 flex flex-col z-30 shadow-2xl animate-in slide-in-from-right duration-200'>
         ...
         <div className="p-4 border-b border-slate-200 flex items-center justify-between">
          ...
          <button onClick={onClose} className="p-1 rounded-lg hover:bg-slate-100 text-slate-500 hover:text-slate-700 transition-colors cursor-pointer" aria-label="Close chat">
            ...
            messages.map((msg, index)=>{
              ...
              return (
                <div key={msg.id || index} className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                  ...
                  <div className={`px-3.5 py-2.5 rounded-2xl max-w-[85%] text-sm leading-relaxed shadow-xs ${isMe ? "bg-primary text-white rounded-tr-none font-medium" : "bg-slate-100 text-slate-800 rounded-t1-none border border-slate-200 font-medium"}`}>
                    {msg.text}
                  </div>

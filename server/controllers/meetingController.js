...
export const createMeeting = async (req, res)=> {
    try{
        ...
        const hostName = user[0]?.name || "Host";
...
//Get all users meetings sessions
export const getUserSessions = async (req, res)=> {
    try{
        ...
        SELECT DISTINCT m.id, m.meeting_id, m.title, m.status, m.created_at, m.ended_at, m.host_id, u.name as host_name, u.email as host_email FROM meetings m 
        ...
        const formattedMeetings = await Promise.all(
            meetings.map(async (m) => {
                ...
                SELECT mp.*, userId.email
                FROM meeting_participants mp
                LEFT JOIN users u ON mp.user_id = u.id
                WHERE mp.meeting_id = ${m.id}`;

                const messages = await sql`
                SELECT id, sender_id, sender_nme, text, timestamp
...
export const getSessionDetails = async (req, res)=> {
    try{
        ...
        const meetings = await sql`SELECT m.*, u.id AS host_user_id,
        u.name AS host_name,
